...
const PokemonForm: FunctionComponent<Props> = ({ pokemon, isEditForm }) => {
  ...
  const selectType = (
    ...
    // IF => User check a type, add to current pokemon types list
    if (checked) {
      const newTypes: string[] = form.types.value.concat([type]);
      newField = { value: newTypes };
    } else {
      // ELSE => User uncheck a type, remove it to current pokemon types list
  ...
  const addPokemon = () => {
    PokemonServices.addPokemon(pokemon).then(() => history.push("/pokemons"));
  };

  const updatePokemon = () => {
    PokemonServices.updatePokemon(pokemon).then(() =>
      history.push(`/pokemons/${pokemon.id}`)
    );
  };

  const deletePokemon = () => {
    PokemonServices.deletePokemon(pokemon).then(() =>
  ...
  const isTypeValid = (type: string): boolean => {
    // IF => Disallow a user to uncheck last type selected
    if (form.types.value.length === 1 && hasType(type)) {
      return false;
    }

    // IF => Disallow a user to check more than 3 types
